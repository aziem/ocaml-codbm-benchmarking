...
run_framac_benchmarks () {
 ...
 do
     ...
     cd $loc; ./runeva.sh $NUMRUNS &> ${currentdir}/${name}$1.txt
     cat ${currentdir}/${name}$1.txt | grep -A1 user | awk '{print $2}' | awk '{sum+=$1} END {print sum}' > ${currentdir}/${name}$1-mean.txt
     cat ${currentdir}/${name}$1.txt | grep -A1 user | awk '{print $5}' | awk '{sum+=$1} END {print sum}' > ${currentdir}/${name}$1-median.txt
     echo "Done"
 done
}
